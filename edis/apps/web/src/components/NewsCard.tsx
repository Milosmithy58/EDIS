...
const NewsCard = ({ geo, query, country }: Props) => {
  const {
    data,
    isFetching,
    isError,
  ...
  return (
    <article className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      ...
      {!geo && <p className="text-sm text-slate-500">Select a location to see local news.</p>}
      {geo && !query && (
        <p className="text-sm text-slate-500">We will search once you pick a place.</p>
      )}
      {geo && query && isFetching && <p className="text-sm text-slate-500">Loading news…</p>}
      {geo && query && isError && (
